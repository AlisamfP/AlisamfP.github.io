    ...
    [
        {
            ...
            description:
                "Open Flow is a free, open-source AAC communication tool built for people with speech or language challenges. Originally my senior project, it was inspired by my own use of physical communication cards and how often I lose them. Built with React, TypeScript, and now backed by a real database and user authentication.",
            github: "https://github.com/AlisamfP/openflow",
            projectLink: "https://openflow.palson.info/",
            background:
                "AAC, or Augmentative and Alternative Communication, refers to all the ways people communicate without speaking. Many who have trouble communicating use AAC tools like picture boards with images and words to help express themselves. Open Flow is a digital version of an AAC tool I personally use: communication cards. It's essentially a web app where users can click on cards to speak phrases out loud using text-to-speech. Users can favorite commonly used cards, create their own custom cards, adjust voice settings, or switch to an audio off mode where cards display full screen instead. As an autistic adult, this project is personal to me, and that perspective influenced the design's focus on comfort, clarity, and reducing sensory overload.",
            problem:
                "Many AAC apps either lean heavily towards childlike design, or sit on the opposite end of the spectrum with designs that feel clinical and lacking personality. Many are also expensive, restricted by proprietary systems, or limited in functionality. This makes it hard for autistic teens and adults to find tools that feel good to use and meet their needs. I set out to create an open source, low stimulation AAC app that feels accessible, flexible, and designed with autistic users in mind.",
            process:
                "I started by researching existing AAC devices and apps to understand what common phrases and features users need most. Research into autistic design best practices influenced the soft color palette, layout conventions, and font choice. After sketching wireframe options, I built out a mockup and started development.<br>The first version was submitted as my senior project and built with HTML, CSS, and JavaScript, using jQuery plugins for UI components and localStorage to save user preferences and custom cards. I selected OpenMoji, an open source emoji set, for card icons since its open nature aligned with the project's values.<br>After completing the initial version, I rebuilt it in React with TypeScript and Vite for version 2, switching from Tailwind CSS to Material UI for more consistent and predictable UI patterns. I replaced ResponsiveVoice with the built-in Web Speech API and continued taking an iterative, modular approach throughout.<br>Version 3 is the biggest update yet. I migrated the entire project from Vite to Next.js, moved from GitHub Pages to Vercel, and replaced localStorage with a MongoDB database. I added full user authentication using Better Auth and built out all the API routes and database structure myself. While I have worked as a full stack engineer before, this was the first time I owned the database design from scratch rather than working with existing systems and endpoints. Thinking through the data relationships, schema design, and API architecture from the ground up was a different kind of challenge, and one I really enjoyed.",
            results:
                "Open Flow has been one of the most rewarding projects I have worked on. Each version has pushed me in a different direction. The earlier versions were about getting comfortable with modern frontend tooling and component-based architecture. Version 3 was about stepping back into backend territory with fresh eyes and taking full ownership of decisions I had previously left to others or inherited from existing systems. Designing the database structure, thinking through relationships between collections, and building an API that actually made sense for the app gave me a lot more confidence in that side of development. The app functions well and continues to reflect both my personal experience as an autistic adult and my growth as a developer. Development is ongoing and I am genuinely excited to keep building on it.",
            images: [
                {
        ...
        },
        {
            ...
            description:
                "A digital Pokédex that lets users browse, search, and save Pokémon to their own team. Built with vanilla HTML, CSS, and jQuery, it uses the PokéAPI to pull real-time data and includes features like autocomplete search, sprite slideshows, and stat tabs. Inspired by the classic game interface, the project combines nostalgic design with smooth, modern functionality.",
            github: "https://github.com/AlisamfP/pokedex",
            projectLink: "http://alisa.palson.info/pokedex/",
            background:
                "Inspired by the iconic Pokédex from the Pokémon games, I set out to build a web app that lets users explore, search, and save Pokémon to their personal team. The goal was to create an interactive site that feels like a digital Pokédex brought to life.",
            problem:
